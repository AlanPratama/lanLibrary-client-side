...
export default function Type() {
  ...
  const addTypeSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8000/api/libManager/type`,
        {
          name: name,
        },
        {
  ...
  const handleEditTypeModal = async (slug) => {
    if (!editTypeModal) {
      setSlug(slug);

      const res = await axios.get(`http://localhost:8000/api/type/${slug}`, {
        withCredentials: true,
      });
  ...
  const editTypeSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8000/api/libManager/type/${slug}`,
        {
          name: name,
        },
        {
      ...
      console.log({ errrrrorrr: response });
  ...
  const handleDelTypeModal = (slug) => {
    if (!delTypeModal) {
      setSlug(slug)
      setDelTypeModal(true)
    } else {
      setSlug("")
  ...
  const delTypeSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await axios.delete(`http://localhost:8000/api/libManager/type/${slug}`, {
        withCredentials: true
      })
  ...
  return (
    <>
      ...
      <table className="min-w-full  bg-white font-[sans-serif]">
        ...
        <tbody className="whitespace-nowrap">
          {type
            ? type.map((type, index) => {
                return (
                  <tr className="" key={type.id}>
                    <td className="pl-6 w-8">{index + 1}</td>
                    <td className="px-6 py-3 text-sm">{type.name}</td>
                    <td className="px-6 py-3 text-sm">{type.total_book}</td>
                    <td className="px-6 py-3">
                      <button
                        className="mr-4"
                        title="Edit"
                        onClick={() => handleEditTypeModal(type.slug)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                      ...
                      <button
                        className="mr-4"
                        title="Delete"
                        onClick={() => handleDelTypeModal(type.slug)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
      ...
      >
        <div className="relative w-full flex justify-center items-center h-full">
          <div className="max-w-4xl max-h-full rounded-lg bg-white p-8 shadow-lg lg:col-span-3 lg:p-12">
            <form className="space-y-4" onSubmit={addTypeSubmit}>
      ...
      <div
        className={`${
          !editTypeModal ? "hidden" : ""
        }  l transition-all min-h-screen h-auto overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 max-h-full`}
      >
        <div className="relative w-full flex justify-center items-center h-full">
          <div className="max-w-4xl max-h-full rounded-lg bg-white p-8 shadow-lg lg:col-span-3 lg:p-12">
            <form className="space-y-4" onSubmit={editTypeSubmit}>
              <div className="grid grid-cols-1 gap-4 text-center">
                <div>
                  <label className="sr-only" htmlFor="name">
                    Type Name
      ...
      >
        <div className="relative w-full flex justify-center items-center h-full">
          <div className="max-w-4xl max-h-full rounded-lg bg-white p-8 shadow-lg lg:col-span-3 lg:p-12">
            <form className="space-y-4" onSubmit={delTypeSubmit}>
            <div className="grid grid-cols-1 gap-4 text-center">
                <div>
                  <h3 className="text-lg">
                    Apakah Kamu Yakin Akan Menghapus Type Ini?
                  </h3>
                </div>
              </div>
