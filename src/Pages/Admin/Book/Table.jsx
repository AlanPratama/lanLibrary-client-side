  ...
  const handleEditBookModal = async (bookSlug) => {
    if (!editBookModal) {
      const response = await axios.get(
        `http://localhost:8000/api/book/${bookSlug}`,
        {},
        {
  ...
  const editBookSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:8000/api/libManager/book/${bookSlug}`,
        formEditData,
        {
  ...
  const delBookSubmit = async (e) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:8000/api/libManager/book/${bookDelSlug}`,
        {
          withCredentials: true,
        }
      );
                  ...
                  <tr className="" key={book.id}>
                    ...
                    <td className="px-6 py-3">
                      <button
                        className="mr-4"
                        title="Edit"
                        onClick={() => handleEditBookModal(book.slug)}
                      >
                        <svg
                      ...
                      <button
                        className="mr-4"
                        title="Delete"
                        onClick={() => handleDelBookModal(book.slug)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="max-w-4xl max-h-full rounded-lg bg-white p-8 shadow-lg lg:col-span-3 lg:p-12">
            ...
            >
              <h3 className="text-xl text-deep-purple-accent-400 font-medium">
                Apakah Kamu Yakin Akan Menghapus Buku Ini?
              </h3>

              <div className="w-full mt-4 flex justify-start items-start gap-3">
                <img
                  src={`http://localhost:8000${bookDelCover}`}
                  alt={bookDelCover}
                  className="shadow rounded min-w-36 max-w-36"
                />
